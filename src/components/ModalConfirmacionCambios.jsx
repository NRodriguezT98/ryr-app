...
const ModalConfirmacionCambios = ({ isOpen, onClose, onConfirm, titulo, cambios = [], isSaving = false }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[60] animate-fade-in">
            <div className="bg-white p-7 rounded-2xl shadow-2xl max-w-md w-full mx-4 flex flex-col">
                ...
                {cambios.length > 0 ? (
                    ...
                    <p className="text-center text-gray-500 mb-6">No se detectaron cambios específicos, pero se guardará el estado actual.</p>
                )}

                <div className="flex justify-end gap-4 mt-auto">
                    <button onClick={onClose} disabled={isSaving} className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold px-5 py-2 rounded-lg transition-colors duration-200 disabled:opacity-50">
                        Cancelar
                    </button>
                    <button onClick={onConfirm} disabled={isSaving} className="bg-green-600 hover:bg-green-700 text-white font-semibold px-5 py-2 rounded-lg shadow-md transition-colors duration-200 disabled:bg-gray-400">
