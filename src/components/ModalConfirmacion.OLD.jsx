...
// Función para obtener el icono y color según el tipo de campo
const getFieldIcon = (campo) => {
    // Vivienda
    if (campo.includes('Vivienda') || campo.includes('Manzana') || campo.includes('Casa')) {
        return { Icon: Home, color: 'text-blue-500 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20' };
    }
    ...
    if (campo.includes('Nombres') || campo.includes('Apellidos') || campo.includes('Documento') || 
        ...
        return { Icon: User, color: 'text-purple-500 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20' };
    }
    
    // Archivos / Documentos
    if (campo.includes('Cédula') || campo.includes('Carta de Aprobación')) {
        return { Icon: FileCheck, color: 'text-emerald-500 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-900/20' };
    }
    
    // Datos financieros - Montos
    if (campo.includes('Cuota Inicial') || campo.includes('Cuota Mensual') || campo.includes('Monto')) {
        return { Icon: DollarSign, color: 'text-green-500 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-900/20' };
    }
    
    // Crédito
    if (campo.includes('Crédito Hipotecario')) {
        return { Icon: CreditCard, color: 'text-indigo-500 dark:text-indigo-400', bg: 'bg-indigo-50 dark:bg-indigo-900/20' };
    }
    
    // Subsidios
    if (campo.includes('Subsidio')) {
        return { Icon: Building2, color: 'text-cyan-500 dark:text-cyan-400', bg: 'bg-cyan-50 dark:bg-cyan-900/20' };
    }
    
    // Banco / Caja
    if (campo.includes('Banco') || campo.includes('Caja')) {
        return { Icon: Briefcase, color: 'text-amber-500 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/20' };
    }
    
    // Tasa de interés
    if (campo.includes('Tasa')) {
        return { Icon: Percent, color: 'text-rose-500 dark:text-rose-400', bg: 'bg-rose-50 dark:bg-rose-900/20' };
    }
    
    // Plazo
    if (campo.includes('Plazo')) {
        return { Icon: Calendar, color: 'text-teal-500 dark:text-teal-400', bg: 'bg-teal-50 dark:bg-teal-900/20' };
    }
    
    // Referencia
    if (campo.includes('Referencia')) {
        return { Icon: Hash, color: 'text-slate-500 dark:text-slate-400', bg: 'bg-slate-50 dark:bg-slate-900/20' };
    }
    
    // Default
    return { Icon: TrendingUp, color: 'text-gray-500 dark:text-gray-400', bg: 'bg-gray-50 dark:bg-gray-900/20' };
};
                        ...
                        {cambios.map((cambio, index) => {
                            ...
                            return (
                                <div key={index} className="p-2 border-b border-gray-200 dark:border-gray-600 last:border-b-0 animate-fade-in">
                                    ...
                                    {cambio.mensaje ? (
                                        // Si hay mensaje personalizado, mostrarlo
                                        <div className="mt-2">
                                        ...
                                        // Renderizado especial para archivos sin mensaje
