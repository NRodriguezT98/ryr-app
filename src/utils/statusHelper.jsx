...
export const determineClientStatus = (cliente) => {
    ...
    if (s.pagoBoletaRegistro?.completado) return STATUS_CONFIG.procesoRegistro;
    if (s.minutaFirmada?.completado) return STATUS_CONFIG.firmaEscritura;
    if (s.pagoEstudioTitulos?.completado) return STATUS_CONFIG.estudioTitulos;
    if (s.envioDocumentacionAvaluo?.completado) return STATUS_CONFIG.procesoAvaluo;
