...
const EditarAbonoModal = ({ isOpen, onClose, onSave, abonoAEditar }) => {
    ...
    const isBotonRevisarDeshabilitado = !hayCambios || Object.keys(errors).length > 0 || isSubmitting;
    ...
    return (
        <>
                ...
                <form onSubmit={handleSubmit} noValidate>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                                <div className="text-center p-2 bg-gray-100 dark:bg-gray-700 rounded-md w-1/2">
                                    ...
                                    <p className="font-semibold text-gray-600 dark:text-gray-300 line-through">{formatCurrency(abonoAEditar?.monto)}</p>
                                </div>
                        ...
                        <div>
                            ...
                            <input
                                type="date" id="fechaPago-edit" name="fechaPago" value={formData.fechaPago || ''} onChange={handleInputChange}
                                max={getTodayString()} min={abonoEnriquecido?.cliente?.datosCliente?.fechaIngreso?.split('T')[0]}
                        ...
                        <div>
                            ...
                            <textarea
                                id="observacion-edit" name="observacion" value={formData.observacion || ''} onChange={handleInputChange}
                                rows="3" className="mt-1 block w-full shadow-sm sm:text-sm border rounded-md dark:bg-gray-700 dark:text-white px-3 py-2 border-gray-300 dark:border-gray-600"
                            ...
                            {formData.urlComprobante ? (
                                <div className="bg-green-50 dark:bg-green-900/50 border-2 border-green-200 dark:border-green-700 rounded-lg p-4 flex items-center justify-between">
                                    <div className='flex items-center gap-2 text-green-800 dark:text-green-300 font-semibold'>
                                        <FileText /><a href={formData.urlComprobante} target="_blank" rel="noopener noreferrer" className="hover:underline">Ver Comprobante Actual</a>
                                    </div>
                                    <button type="button" onClick={() => handleValueChange('urlComprobante', null)} className="p-1 text-red-500 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50" title="Eliminar comprobante"><XCircle size={20} /></button>
                                </div>
                            ) : (
                                <FileUpload
                    ...
                    <div className="mt-6 pt-4 border-t dark:border-gray-600 sm:flex sm:flex-row-reverse items-center">
                        <div className="relative">
                            <button
                                type="submit"
                                disabled={isBotonRevisarDeshabilitado}
                            ...
                            {isBotonRevisarDeshabilitado && (
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max bg-gray-700 text-white text-xs rounded py-1 px-2 pointer-events-none opacity-90">
                                    {!hayCambios ? "No has realizado cambios" : (Object.keys(errors).length > 0 ? "Hay errores en el formulario" : "")}
                                </div>
                            )}
                        </div>
            ...
            <ModalConfirmacion
                ...
                titulo="Confirmar Cambios del Abono"
                cambios={cambios}
