...
const EditarCliente = ({ isOpen, onClose, onGuardar, clienteAEditar }) => {
    ...
    const viviendasDisponibles = useMemo(() => {
        ...
        const todas = getViviendas();
        return todas.filter(v => v.clienteId === null || v.clienteId === clienteAEditar.id);
    }, [clienteAEditar]);

    // --- CORRECCIÓN DE ORDEN 1: El hook useForm se declara primero ---
    const {
        formData,
        setFormData,
        errors,
    ...
    // --- CORRECCIÓN DE ORDEN 2: Las funciones y memos que usan `formData` se declaran DESPUÉS de useForm ---
    ...
    const handleFinalSave = useCallback(() => {
        if (updateCliente(clienteAEditar.id, formData)) {
    ...
    const handleSubmitWithConfirmation = useCallback((formData) => {
        const viviendaOriginal = getViviendas().find(v => v.id === clienteAEditar.viviendaId);
        const viviendaNueva = getViviendas().find(v => v.id === formData.viviendaId);
    ...
    }, [clienteAEditar, onClose, formData]);
