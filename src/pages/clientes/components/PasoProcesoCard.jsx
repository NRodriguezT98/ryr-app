    ...
    return (
        <div className={cardClasses}>
            <div className="flex items-start justify-between">
                ...
                {data?.completado && (
                    <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-gray-700">{formatDisplayDate(data.fecha)}</p>
                        {!isPermanentlyLocked && (
                            <>
                                <button onClick={() => onIniciarEdicionFecha(key)} disabled={hayPasoEnReapertura} className="p-1 text-gray-500 hover:text-blue-600 rounded-full disabled:text-gray-300 disabled:cursor-not-allowed" title="Editar fecha y motivo"><Pencil size={16} /></button>
                                <button onClick={() => onIniciarReapertura(key)} disabled={isLocked || hayPasoEnReapertura} className="p-1 text-gray-500 hover:text-blue-600 rounded-full disabled:text-gray-300 disabled:cursor-not-allowed" title="Reabrir este paso"><RotateCcw size={16} /></button>
                            </>
                        )}
                    </div>
                )}
            ...
            {!data?.completado && !isLocked && (
                <div className="mt-4 pl-9 space-y-3">
                    <div className="flex justify-between items-center"><p className="text-sm font-semibold text-gray-600">Evidencias Requeridas:</p><span className={`text-xs font-bold px-2 py-1 rounded-full ${evidenciasSubidas === totalEvidencias ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{`${evidenciasSubidas} de ${totalEvidencias} subidas`}</span></div>
                    ...
                    {puedeCompletarse && (
                        <div className="flex items-center justify-end gap-3 pt-3 border-t">
                            <button onClick={() => onDeshacerReapertura(key)} className="bg-gray-200 text-gray-800 font-semibold px-4 py-2 rounded-lg hover:bg-gray-300 flex items-center gap-2"><X size={16} /> Cancelar</button>
                            <input type="date" value={fechaCompletado} onChange={(e) => setFechaCompletado(e.target.value)} min={paso.minDate} max={paso.maxDate} className="text-sm border p-1.5 rounded-md" />
            ...
            {error && <div className="mt-2 pl-9 flex items-center gap-2 text-red-600 text-xs font-semibold"><AlertCircle size={14} /><p>{error}</p></div>}
        </div>
    );
};
