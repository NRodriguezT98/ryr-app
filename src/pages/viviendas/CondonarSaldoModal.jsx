...
const CondonarSaldoModal = ({ isOpen, onClose, onSave, vivienda }) => {
    const {
        formData,
        errors,
        isSubmitting,
        handlers
    } = useCondonarSaldo(vivienda, onSave, onClose);
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title="Condonar Saldo Pendiente"
            icon={<HandCoins size={28} className="text-green-500" />}
        >
            <form onSubmit={handlers.handleSubmit}>
                <div className="space-y-6">
                    <p className="text-center text-gray-500 dark:text-gray-400 -mt-4">
                        {`Se registrará un abono por el saldo restante de la vivienda Mz ${vivienda.manzana} - Casa ${vivienda.numeroCasa}.`}
                    </p>
                    <div className="bg-blue-50 dark:bg-blue-900/50 border border-blue-200 dark:border-blue-800 p-4 rounded-lg text-center">
                        ...
                        <p className="text-3xl font-bold text-blue-700 dark:text-blue-400">{formatCurrency(vivienda.saldoPendiente)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        {formData.urlSoporte ? (
                            ...
                            <FileUpload
                                ...
                                filePath={(fileName) => `documentos_condonaciones/${vivienda.id}/${fileName}`}
