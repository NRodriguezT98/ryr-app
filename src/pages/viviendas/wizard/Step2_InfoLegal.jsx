    ...
    return (
        <AnimatedPage>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block font-semibold mb-1 flex items-center" htmlFor="matricula">
                    ...
                    <input id="matricula" name="matricula" type="text" value={formData.matricula} onChange={handleInputChange} className={`w-full border p-3 rounded-lg ${errors.matricula ? "border-red-500" : "border-gray-300"}`} maxLength={15} />
                    {errors.matricula && <p className="text-red-600 text-sm mt-1">{errors.matricula}</p>}
                </div>
                <div>
                    <label className="block font-semibold mb-1 flex items-center" htmlFor="nomenclatura">
                    ...
                    <input id="nomenclatura" name="nomenclatura" type="text" value={formData.nomenclatura} onChange={handleInputChange} className={`w-full border p-3 rounded-lg ${errors.nomenclatura ? "border-red-500" : "border-gray-300"}`} maxLength={25} />
                    {errors.nomenclatura && <p className="text-red-600 text-sm mt-1">{errors.nomenclatura}</p>}
                </div>
                <div>
                    <label className="block font-semibold mb-1 flex items-center" htmlFor="areaLote">
                        Área del Lote (m²) <span className="text-red-600">*</span>
                        <HelpTooltip id="areaLote" content="Area total del lote por ejemplo: 66.125" />
                    </label>
                    <input id="areaLote" name="areaLote" type="text" value={formData.areaLote} onChange={handleInputChange} className={`w-full border p-3 rounded-lg ${errors.areaLote ? "border-red-500" : "border-gray-300"}`} />
                    {errors.areaLote && <p className="text-red-600 text-sm mt-1">{errors.areaLote}</p>}
                </div>
                <div>
                    <label className="block font-semibold mb-1 flex items-center" htmlFor="areaConstruida">
                    ...
                    <input id="areaConstruida" name="areaConstruida" type="text" value={formData.areaConstruida} onChange={handleInputChange} className={`w-full border p-3 rounded-lg ${errors.areaConstruida ? "border-red-500" : "border-gray-300"}`} />
            ...
            <div className="md:col-span-2 mt-6 pt-6 border-t border-dashed">
                ...
                {formData.urlCertificadoTradicion ? (
                    <div className="bg-green-50 border-2 border-green-200 rounded-lg p-4 flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => handleValueChange('urlCertificadoTradicion', null)} className="p-1 text-red-500 rounded-full hover:bg-red-100" title="Eliminar documento"><XCircle size={20} /></button>
                    </div>
                ) : (
                    <FileUpload
